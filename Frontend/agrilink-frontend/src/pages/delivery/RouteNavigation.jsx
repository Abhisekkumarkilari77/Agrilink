...
const RouteNavigation = () => {
  ...
  useEffect(() => {
    ...
    // Initialize Leaflet Map centered around Bengaluru
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-150 p-6 max-w-xl mx-auto shadow-sm space-y-6">
      ...
      <div className="p-4 bg-gray-50 rounded-2xl text-xs space-y-2 border">
        ...
        <p><span className="text-gray-400">Total Distance:</span> <span className="font-bold text-gray-700">{order?.distance || '4.2 km'}</span></p>
        <p><span className="text-gray-400">Est Time to Location:</span> <span className="font-bold text-green-700">{order?.eta || '15 mins'}</span></p>
        <p><span className="text-gray-400">Traffic Condition:</span> <span className="font-bold text-gray-700">Moderate Traffic</span></p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => alert(`Calling Farmer: ${order?.farmerPhone || 'N/A'}`)} className="py-3 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-xl text-xs transition">
          📞 Call Farmer
        </button>
        <button onClick={() => alert(`Calling Customer: ${order?.customerPhone || 'N/A'}`)} className="py-3 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-xl text-xs transition">
          📞 Call Customer
        </button>
      </div>
    </div>
